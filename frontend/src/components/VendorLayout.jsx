...
const VendorSidebar = () => {
  ...
  return (
    <div className="w-64 bg-green-900 min-h-screen">
      <div className="p-6">
      ...
      <nav className="mt-6">
        {menuItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={`flex items-center px-6 py-3 text-gray-300 hover:bg-green-800 hover:text-white ${
              location.pathname === item.path ? 'bg-green-800 text-white border-l-4 border-green-400' : ''
            }`}
          >
